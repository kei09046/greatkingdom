...
void TrainPipeline::start_self_play(MCTSPlayer* player, bool is_shown, float temp, int n_games) {
	...
	while (true) {
		...
		if (cnt < 5) {
			...
			player->get_action(game_manager, win_prob, move_prob, is_shown, temp);
			...
			if (!result) {
				...
				insert_equi_data(TrainPipeline::surprised(player->mcts.pv->policy_value_fn(state[0]), move_prob.second, win_prob), 
					state[0], move_prob.second, win_prob/*, state[1]*/);
			}
...
void TrainPipeline::update()
{
	...
	auto probs_value = policy_value_net.policy_value(state_batch).second;
	...
	for (int i = 0; i < batchSize; ++i) {
		...
		value_memory.update(train_data[i].idx, abs(probs_value[i] - (*winner_batch)[i]));
	}
	...
	for (int i = 0; i < epochs; ++i) {
		policy_value_net.train_step(*state_batch, *mcts_probs, *is_weight, learning_rate);
	}

	auto probs_policy = policy_value_net.policy_value(state_batch).first;
	// 이전 state 와 다음 state의 평가는 반대이므로 -가 아닌 + 필요
	...
	for (int i = 0; i < batchSize; ++i) {
		...
		for (int j = 0; j <= totSize; ++j) {
			...
			temp += (probs_policy[cnt] - (*mcts_probs)[cnt]) * (probs_policy[cnt] - (*mcts_probs)[cnt]);
			cnt++;
		}
